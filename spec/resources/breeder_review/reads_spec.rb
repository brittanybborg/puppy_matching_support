require 'rails_helper'

RSpec.describe BreederReviewResource, type: :resource do
  describe 'serialization' do
    let!(:breeder_review) { create(:breeder_review) }

    it 'works' do
      render
      data = jsonapi_data[0]
      expect(data.id).to eq(breeder_review.id)
      expect(data.jsonapi_type).to eq('breeder_reviews')
    end
  end

  describe 'filtering' do
    let!(:breeder_review1) { create(:breeder_review) }
    let!(:breeder_review2) { create(:breeder_review) }

    context 'by id' do
      before do
        params[:filter] = { id: { eq: breeder_review2.id } }
      end

      it 'works' do
        render
        expect(d.map(&:id)).to eq([breeder_review2.id])
      end
    end
  end

  describe 'sorting' do
    describe 'by id' do
      let!(:breeder_review1) { create(:breeder_review) }
      let!(:breeder_review2) { create(:breeder_review) }

      context 'when ascending' do
        before do
          params[:sort] = 'id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            breeder_review1.id,
            breeder_review2.id
          ])
        end
      end

      context 'when descending' do
        before do
          params[:sort] = '-id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            breeder_review2.id,
            breeder_review1.id
          ])
        end
      end
    end
  end

  describe 'sideloading' do
    # ... your tests ...
  end
end
